...
?>
<?php else:  ?>
<form class="max-w-sm mx-auto" action="../actions/signup-user.php" method="POST">
    <h1 class="text-white text-center">Register for new account</h1>
    <div class="mb-5">
        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your name</label>
        <input type="name" id="name" name="name" value="<?php if (isset($_GET["name"])) {
                                                                echo $_GET["name"];
                                                            } ?>"
    ...
    <?php
        if (isset($_GET["err"])):
            $err = $_GET["err"];
        ?>
    <p class="text-red-500 my-4"><?= $err ?></p>
    <?php
        endif;
        ?>
